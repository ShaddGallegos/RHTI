...
echo '
                                ANSIBLE-TOWER 3.8.5-3 INSTALLER FOR RHEL 7.x AND RHEL 8.x
...
echo "
                        ...
                        6. This install was tested with:
                          ...
                          * Ansible Tower 3.8.5-3 https://releases.ansible.com/ansible-tower/setup-bundle/ansible-tower-setup-bundle-latest.el8.tar.gz (will auto download)
                          * Red Hat subscriber channels:
...
cd ~/Downloads
echo "***************************************************************************"
echo "SET SELINUX TO PERMISSIVE FOR THE INSTALL AND CONFIG OF Ansible Tower 3.8.5-3"
echo "***************************************************************************"
...
echo "*******************"
subscription-manager repos --enable rhel-7-server-extras-rpms --enable rhel-7-server-optional-rpms --enable rhel-7-server-supplementary-rpms --enable rhel-server-rhscl-7-rpms --enable rhel-7-server-rpms --enable rhel-7-server-ansible-2.9-rpms --enable automation-hub-4.2-for-rhel-8-x86_64-rpms
...
yum -q list installed screen &>/dev/null && echo "screen is installed" || yum install -y screen --skip-broken --noplugins --best --allowerasing
...
echo '*********************************************'
usermod -a -G root awx
dnf install -y python3-pip ansible ansible-doc --skip-broken --best --allowerasing 
source /var/lib/awx/venv/ansible/bin/activate
umask "0022"
 sudo -u  awx pip3 install --user --upgrade pip boto3 ansible-tower-cli boto botocore requests requests-credssp cryptography pywinrm PyVmomi azure-mgmt-compute azure-mgmt-resource azure-keyvault-secrets six netaddr passlib
deactivate
}
...
then 
echo '****************************************************************'
echo 'Getting, Expanding, and installing Ansible Tower 3.8.5-3 for RHEL7'
echo '****************************************************************'
mkdir ~/Downloads
cd ~/Downloads
wget https://releases.ansible.com/ansible-tower/setup-bundle/ansible-tower-setup-bundle-3.8.5-3.tar.gz
tar -zxvf ~/Downloads/ansible-tower-setup-bundle-3.8.5-3.tar.gz 
cd ~/Downloads/ansible-tower-setup-bundle-3.8.5-3
dnf localinstall -y --skip-broken ~/Downloads/ansible-tower-setup-bundle-3.8.5-3/bundle/el7/repos/ansible-tower-dependencies/*.rpm  --best --allowerasing
sleep 2
echo " "
echo " " 
cd 
...
read ADMINPASSWORD
export $ADMINPASSWORD
sed -i 's/admin_password='"''"'/admin_password='"'"$ADMINPASSWORD"'"'/g' ~/Downloads/ansible-tower-setup-bundle-3.8.5-3/inventory
sed -i 's/pg_password='"''"'/pg_password='"'"$ADMINPASSWORD"'"'/g' ~/Downloads/ansible-tower-setup-bundle-3.8.5-3/inventory
sed -i 's/automationhub_admin_password='"''"'/automationhub_admin_password='"'"$ADMINPASSWORD"'"'/g' ~/Downloads/ansible-tower-setup-bundle-3.8.5-3/inventory
sed -i 's/automationhub_pg_password='"''"'/automationhub_pg_password='"'"$ADMINPASSWORD"'"'/g' ~/Downloads/ansible-tower-setup-bundle-3.8.5-3/inventory
...
then
echo '****************************************************************'
echo 'Getting, Expanding, and installing Ansible Tower 3.8.5-3 for RHEL8'
echo '****************************************************************'
mkdir ~/Downloads
cd ~/Downloads
wget https://releases.ansible.com/ansible-tower/setup-bundle/ansible-tower-setup-bundle-3.8.5-3.tar.gz
tar -zxvf ~/Downloads/ansible-tower-setup-bundle-3.8.5-3.tar.gz 
cd ~/Downloads/ansible-tower-setup-bundle-3.8.5-3
dnf localinstall -y --skip-broken ~/Downloads/ansible-tower-setup-bundle-3.8.5-3/bundle/el8/repos/ansible-tower-dependencies/*.rpm --best --allowerasing
sleep 2
echo " "
echo " "
