...
echo '
                                ANSIBLE-TOWER 3.8.2-1 INSTALLER FOR RHEL 7.x AND RHEL 8.x
...
subscription-manager attach --pool=$(subscription-manager list --available --matches 'Red Hat Ansible Automation, Standard (100 Managed Nodes)' |grep 'Pool ID:' |awk '{print $3}')
else
echo "System is all ready registered with Red Hat or a Red Hat Satellite, Continuing!"
sleep 2 
fi
echo " "
echo " "
...
echo '*******************'
subscription-manager repos --disable '*'
subscription-manager repos --enable ansible-2.9-for-rhel-8-x86_64-rpms --enable rhel-8-for-x86_64-appstream-rpms --enable rhel-8-for-x86_64-baseos-rpms --enable rhel-8-for-x86_64-supplementary-rpms --enable automation-hub-4.2-for-rhel-8-x86_64-rpms 
