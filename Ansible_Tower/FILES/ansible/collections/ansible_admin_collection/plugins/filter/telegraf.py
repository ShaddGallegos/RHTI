...
def config(config, exclude=[]):
    if not isinstance(config, dict):
        raise AnsibleFilterError('telegraf_config expects a dict but was given a %s' % type(config))
...
def config_parameters(parameters, exclude=[]):
    if not isinstance(parameters, dict):
        raise AnsibleFilterError('telegraf_config_parameters expects a dict but was given a %s' % type(parameters))
...
def config_parameter(parameters, key, default=None, comment=False):
    if not isinstance(parameters, dict):
        raise AnsibleFilterError('telegraf_config_parameter parameters expects a dict but was given a %s' % type(parameters))
    if not isinstance(key, string_types):
        raise AnsibleFilterError('telegraf_config_parameter key expects a string but was given a %s' % type(key))
    result = encode_toml({
...
class FilterModule(object):
    ...
    def filters(self):
        filters = {
            'telegraf_config': config,
            'telegraf_config_parameters': config_parameters,
            'telegraf_config_parameter': config_parameter,
        }
