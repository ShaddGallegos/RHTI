...
def config(config, exclude=[]):
    if not isinstance(config, dict):
        raise AnsibleFilterError('supervisor_config expects a dict but was given a %s' % type(config))
...
def config_parameters(parameters, exclude=[]):
    if not isinstance(parameters, dict):
        raise AnsibleFilterError('supervisor_config_parameters expects a dict but was given a %s' % type(parameters))
...
def config_parameter(parameters, key, default=None, comment=False):
    ...
    if value is True:
        ...
        AnsibleFilterError('supervisor_config_parameter value of an unknown type %s' % type(value))
...
class FilterModule(object):
    ...
    def filters(self):
        filters = {
            ...
            'supervisor_config_parameters': config_parameters,
